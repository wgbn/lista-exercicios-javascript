var meuApp = {
                ...
                render: function () {
                    ...
                    return (
                        React.createElement("div", {className: "item", id: "tarefa-"+this.props.chave, "data-id": this.props.chave},
                            ...
                            React.createElement("div", {className: "texto ", onClick: meuApp.marcarTarefa, "data-id": this.props.chave}, this.props.tarefa),
                            React.createElement("div", {className: "del", "data-id": this.props.chave}, "X")
                        )
                    );
                }
        });
    ...
    marcarTarefa: function(e){
        console.info(e.target.children[1]);
        ...
        if (e.target.className == 'del'){
            ...
            e.target.children[1].setAttribute('class', 'texto'+(meuApp._lista[e.target.dataset.id].status ? ' concluido':''));

            // altera a cor do status da tarefa
            e.target.children[0].setAttribute('class', 'status '+(meuApp._lista[e.target.dataset.id].status ? 'fundo-verde':'fundo-vermelho'));
        }
